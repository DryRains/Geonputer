...
public class CPU {
	...
	public enum Inturrupt{
		Print("p"),
		Switch("s");
		
		private String inturruptInstruction;
		private Inturrupt(String inturruptInstruction) {
			this.inturruptInstruction = inturruptInstruction;
		}
		
		private static Inturrupt getInturruptInstruction(String inturruptInstruction) {
			for (Inturrupt inturrupt : Inturrupt.values()) {
			    if (inturrupt.inturruptInstruction.equals(inturruptInstruction)) {
			        return inturrupt;
			    }
			}
			return null;
	...
	public void associate(IODevice ioDevice) {this.ioDevice = ioDevice;}
	...
	private void execute() {
		...
		case ADD :
			...
			break;
		case SUB :
			if(IR.getMode()==0) AC.setValue(AC.getValue()-IR.getOperand());
			if(IR.getMode()==2) AC.setValue(AC.getValue()-memory.getCurrentProcess().segment[memory.getCurrentProcess().DS+IR.getOperand()]);
			if(IR.getMode()==4) AC.setValue(AC.getValue()-memory.getCurrentProcess().segment[memory.getCurrentProcess().SS+IR.getOperand()]);
			if(IR.getMode()==6) AC.setValue(AC.getValue()-memory.getCurrentProcess().segment[memory.getCurrentProcess().HS+IR.getOperand()]);
			System.out.println("SUB. current value in AC: "+AC.getValue());
			break;
		case MUL :
			if(IR.getMode()==0) AC.setValue(AC.getValue()*IR.getOperand());
			if(IR.getMode()==2) AC.setValue(AC.getValue()*memory.getCurrentProcess().segment[memory.getCurrentProcess().DS+IR.getOperand()]);
			if(IR.getMode()==4) AC.setValue(AC.getValue()*memory.getCurrentProcess().segment[memory.getCurrentProcess().SS+IR.getOperand()]);
			if(IR.getMode()==6) AC.setValue(AC.getValue()*memory.getCurrentProcess().segment[memory.getCurrentProcess().HS+IR.getOperand()]);
			System.out.println("MUL. current value in AC: "+AC.getValue());
			break;
		case DIV :
			if(IR.getMode()==0) AC.setValue(AC.getValue()/IR.getOperand());
			if(IR.getMode()==2) AC.setValue(AC.getValue()/memory.getCurrentProcess().segment[memory.getCurrentProcess().DS+IR.getOperand()]);
			if(IR.getMode()==4) AC.setValue(AC.getValue()/memory.getCurrentProcess().segment[memory.getCurrentProcess().SS+IR.getOperand()]);
			if(IR.getMode()==6) AC.setValue(AC.getValue()/memory.getCurrentProcess().segment[memory.getCurrentProcess().HS+IR.getOperand()]);
			System.out.println("DIV. current value in AC: "+AC.getValue());
			break;
		case EQ : 
			if(IR.getMode()==0) {
		...
		case PUSHINT :
			if(IR.getMode()==4) {
				index = IR.getOperand();
				memory.store(currentProcessNumber,index+memory.getCurrentProcess().SS,0);
				SP.setValue(SP.getValue()+4);
				System.out.println("PUSH INT VAR to Stack Segment["+index+"]");
			}
			break;
		case STF:
			if(IR.getMode()==4) {
				index = IR.getOperand();
				//Free Stack from operand's index to SP
				memory.getCurrentProcess().stackFree(index+memory.getCurrentProcess().SS,SP.getValue());
				//SP <= operand's index
				SP.setValue(index);
				System.out.println("Stack Free. Current Stack Pointer: "+SP.getValue());
			}
			break;
		case NEWINT:
			if(IR.getMode()==6) {
				index = IR.getOperand();
				memory.store(currentProcessNumber,index+memory.getCurrentProcess().HS,0);
				System.out.println("NEW INT VAR to Heap Segment["+index+"]");
			}
			break;
		case PRT :
			...
			break;
		case INP :
			CPU.eState = CPU.EState.ePause;
			memory.ioDevice.submitInput();
			//입력된 값도 memory에 있는 IOBuffer를 통해 간접적으로 읽어옴
			int input = memory.IOIntegerBuffer.lastElement();
			AC.setValue(input);
			memory.IOIntegerBuffer.clear();
			CPU.eState = CPU.EState.eRunning;
			break;
		default:
			break;
		}
	...
	private boolean checkInterrupt() {
		System.out.println("check inturrupt...");
		//IOBuffer에 쌓여있는 인터럽트 체킹
		if(memory.IOBuffer.size()!=0) {
			CPU.eState=CPU.EState.ePause;
			switch(Inturrupt.getInturruptInstruction(memory.IOBuffer.get(0))) {
				case Print:
					System.out.println("<<Inturrupt Type : Print>>\nOperator: "+IR.getOperator()+", Operand: "+IR.getOperand());
					//실행한 인터럽트 명령 버퍼에서 제거
					memory.IOBuffer.remove(0);
					break;
				case Switch:
					System.out.println("<<Inturrupt Type : Switch>>");
					String fileName = loader.selectProgram();
			        contextSwitch(fileName);
			        memory.IOBuffer.clear();//flush
					break;
				default:
					break;
			}
	...
	public void halt() {
//	    // 종료시 메모리에서 제거
//	    memory.remove(memory.getCurrentProcess());
//	    
//	    // 다른 프로그램이 남아있다면 그 프로그램을 실행
//	    if(memory.size()!=0) {
//	    	memory.updateCurruentProcess();
//		    //새로운 프로세스의 PC값, AC값, index 로드
//		    PC.setValue(memory.getCurrentProcess().getPCValue());
//		    AC.setValue(memory.getCurrentProcess().getACValue());
//		    SP.setValue(memory.getCurrentProcess().getSPValue());
//		    this.index = memory.getCurrentProcess().getIndex();
